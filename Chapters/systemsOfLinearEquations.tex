...
\section{Lecture 8: September 9, 2022}
    ...
    \subsection{Solving Linear Systems: Gaussian Elimination}
    
        Given a linear system of equations, we may solve it by the following steps; the following is an informal description of Gaussian elimination.
        \begin{enumerate}
...
\section{Lecture 12: September 19, 2022}
    ...
    \subsection{Inverses of Matrices}
        ...
        \begin{definition}{\Stop\,\,Negative Integral Powers of a Nonsingular Matrix}{nonsingmat}

            Let \(A\) be a nonsingular matrix. Then, the negative integral powers of \(A\) are given as follows: for \(A^{-1}\) the unique inverse of \(A\), we have that for \(k\geq2\), \(A^{-k}=(A^{-1})^k\).
